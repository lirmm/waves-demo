...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('command', choices=['clean_up', 'config', 'init_db'])
        parser.add_argument(
            '--from_date',
            default=None
        )
    ...
    def handle(self, *args, **options):
        # TODO add many more commands :-p
        if options['command'] == 'clean_up':
            removed = []
            for dirname in os.listdir(waves.settings.WAVES_JOB_DIR):
                try:
                    # DO nothing, job exists in DB
                    job = Job.objects.get(slug=uuid.UUID('{%s}' % dirname))
                except ObjectDoesNotExist:
                    removed.append(str(dirname))
                except ValueError:
                    pass
            if len(removed) > 0:
                choice = 1
                while choice not in [2, 3]:
                    ...
                    if choice == 1:
                        ...
                        for dirname in removed:
                            self.stdout.write(os.path.join(waves.settings.WAVES_JOB_DIR, dirname))
                    elif choice == 2:
                        for dirname in removed:
                            self.stdout.write('Removed %s' % dirname)
                            # onerror(os.path.islink, path, sys.exc_info())
                            rmtree(os.path.join(waves.settings.WAVES_JOB_DIR, dirname), ignore_errors=False,
