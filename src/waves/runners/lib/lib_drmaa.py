"""
Subset of sources issued from galaxy pulsar library : https://github.com/galaxyproject/pulsar
@copyright : GalaxyTeam
...
try:
    ...
    LOAD_ERROR_MESSAGE = "RuntimeError - no drmaa system library: %s." % e
    Session = None
    ...
    def close(self):
        with self._session_lock():
            ...
            if DrmaaSession.session_count == 0:
                ...
                log.info("close() exit session.")

    def disconnect(self):
        DrmaaSession.session_count = 0
        DrmaaSession.session.exit()

    def job_info(self, external_job_id):
        with self._session_lock():
            return DrmaaSession.session.wait(str(external_job_id), Session.TIMEOUT_NO_WAIT)
